...
class SimplesamlphpDrupalAuth {
  ...
  public function externalLoginRegister($authname) {
    $account = $this->externalauth->login($authname, 'spid_pasw');
    if (!$account) {
  ...
  public function externalRegister($authname) {
    ...
    if ($existing_user) {
      ...
      if ($this->config->get('autoenablesaml')) {
        ...
        $this->externalauth->linkExistingAccount($authname, 'spid_pasw', $existing_user);
      ...
      if ($this->config->get('autoenablesaml')) {
        ...
        foreach (\Drupal::moduleHandler()->getImplementations('spid_pasw_existing_user') as $module) {
          $return_value = \Drupal::moduleHandler()->invoke($module, 'spid_pasw_existing_user', [$attributes]);
          if ($return_value instanceof UserInterface) {
            ...
            $this->externalauth->linkExistingAccount($authname, 'spid_pasw', $account);
          }
        }
      }
    }
    ...
    if (!$account) {
      // Create the new user.
      try {
        $account = $this->externalauth->register($authname, 'spid_pasw');
      }
      catch (\Exception $ex) {
        watchdog_exception('spid_pasw', $ex);
    ...
    if ($account) {
      ...
      return $this->externalauth->userLoginFinalize($account, $authname, 'spid_pasw');
    }
  }
  ...
  public function getMatchingRoles() {
    ...
    $attributes = $this->simplesamlAuth->getAttributes();
    \Drupal::modulehandler()->alter('spid_pasw_user_roles', $roles, $attributes);
    return $roles;
  }
